...
import io
import safe_text
# A função safe_text robusta deve ser definida aqui ou importada
...
def criar_pdf_relatorio(orcamento_obj, limites, totais_reais, saldo, user_name, frequencia_pagamento) -> bytes:
    ...
    def adicionar_secao_pdf(titulo, total_real, limite, despesas, cor_limite):
    ...
    # Chamadas finais
    adicionar_secao_pdf("50% Necessidades Fixas (Despesas Fixas)", totais_reais['total_fixas'], limites.get('Necessidades (50%)', 0.0), orcamento_obj.despesas_fixas, (144, 238, 144))
    adicionar_secao_pdf("30% Desejos e Lazer (Despesas Variaveis)", totais_reais['total_lazer'], limites.get('Desejos/Lazer (30%)', 0.0), orcamento_obj.gastos_lazer, (173, 216, 230))
    ...
    # SAÍDA FINAL SEGURA E ROBUSTA: Retorna bytes válidos de PDF
    pdf_output = pdf.output(dest='S')
    
    if isinstance(pdf_output, str):
        # fpdf2 em modo 'S' retorna str (latin-1)
        return pdf_output.encode('latin-1') 
    
    # Se fpdf retorna bytes (algumas versões), garante que seja bytes
    if isinstance(pdf_output, (bytes, bytearray)):
        return bytes(pdf_output)
    
    # Caso de falha (não deve acontecer)
    return b'' 
